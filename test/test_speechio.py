...
parse.add_argument(
    "-i", "--input", required=True, help="input wav file or dir of wav files"
)
...
file_handler = logging.handlers.RotatingFileHandler(
    filename=os.path.join(_LOG_FILE_DIR, f"{args.name}.txt")
)
...
def cal_cer(config):
    ...
    _cer_all = 0
    audio_time_total = 0
    transcript_time_total = 0
    for id, info in tqdm(transcripts.items()):
        audio, sample_rate = AudioReader.read_wav_file(
            os.path.join(input, info["path"])
        )
        audio_time_total += len(audio) / 16000

        text = info["text"].replace(" ", "")

        start = time.time()
        hypothesis = model.infer_offline(audio)
        transcript_time_total += time.time() - start
        ...
        logger.info(
            f"id:{id} path: {info['path']}, ground_truth: {text} , hypothesis: {hypothesis}, cer:{error}"
        )
        _cer_all += error

    logger.info(f"all cer is :{_cer_all/len(transcripts.keys())}")
    logger.info(f"rtf is :{transcript_time_total / audio_time_total}")
